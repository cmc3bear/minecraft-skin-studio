...
exports.MASTER_PLAN_OBJECTIVES = void 0;
exports.getObjectiveById = getObjectiveById;
exports.getCriticalObjectives = getCriticalObjectives;
exports.isObjectiveHealthy = isObjectiveHealthy;
exports.getObjectiveHealth = getObjectiveHealth;
exports.calculateObjectiveImpact = calculateObjectiveImpact;
const types_1 = require("./types");
exports.MASTER_PLAN_OBJECTIVES = [
    // Level 1 - Critical Objectives (Non-negotiable)
    {
        id: 'S1',
        level: types_1.ObjectiveLevel.CRITICAL,
    ...
    {
        id: 'C1',
        level: types_1.ObjectiveLevel.CORE,
    ...
    {
        id: 'G1',
        level: types_1.ObjectiveLevel.GROWTH,
        name: 'Active Users',
...
function getObjectiveById(id) {
    return exports.MASTER_PLAN_OBJECTIVES.find(obj => obj.id === id);
}
function getCriticalObjectives() {
    return exports.MASTER_PLAN_OBJECTIVES.filter(obj => obj.level === types_1.ObjectiveLevel.CRITICAL);
}
function isObjectiveHealthy(objective) {
    const value = typeof objective.currentValue === 'number' ? objective.currentValue : parseFloat(objective.currentValue);
...
function getObjectiveHealth(objective) {
    const value = typeof objective.currentValue === 'number' ? objective.currentValue : parseFloat(objective.currentValue);
    const target = typeof objective.target === 'number' ? objective.target : parseFloat(objective.target);
    // For critical objectives, any violation is critical
    if (objective.level === types_1.ObjectiveLevel.CRITICAL && !isObjectiveHealthy(objective)) {
        return 'critical';
    }
    // Calculate deviation from target
    const deviation = Math.abs((value - target) / target);
    if (deviation < 0.05)
        return 'healthy'; // Within 5% of target
    if (deviation < 0.10)
        return 'warning'; // Within 10% of target
    return 'critical'; // More than 10% deviation
}
function calculateObjectiveImpact(objective, projectedValue) {
    const current = typeof objective.currentValue === 'number'
        ? objective.currentValue
        : parseFloat(objective.currentValue);
