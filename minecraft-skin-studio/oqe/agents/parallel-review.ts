...
import { Agent } from '../framework/types';
import { GuardianAgent, ContentFilter, COPPAComplianceChecker, SafetyMonitor } from './guardian';
import { MASTER_PLAN_OBJECTIVES } from '../framework/objectives';
...
export class ParallelAgentReview {
  ...
  private initializeAgents() {
    ...
    this.agents.set('guardian', GuardianAgent);
  ...
  async runParallelReview(projectPath: string): Promise<AgentReview[]> {
  ...
  private async runAgentReview(agent: Agent, projectPath: string): Promise<AgentReview> {
    const startTime = Date.now();
    const findings: Finding[] = [];
    const recommendations: string[] = [];
    const objectiveImpacts: ObjectiveImpact[] = [];
  ...
  private async runGuardianReview(
    projectPath: string, 
    findings: Finding[], 
    recommendations: string[],
    objectiveImpacts: ObjectiveImpact[]
  ...
  private async runPixelPerfectReview(
    projectPath: string,
    findings: Finding[],
    recommendations: string[],
    objectiveImpacts: ObjectiveImpact[]
  ...
  private async runCloudShieldReview(
    projectPath: string,
    findings: Finding[],
    recommendations: string[],
    objectiveImpacts: ObjectiveImpact[]
  ...
  private async runProfessorUXReview(
    projectPath: string,
    findings: Finding[],
    recommendations: string[],
    objectiveImpacts: ObjectiveImpact[]
...
export async function runParallelReview(projectPath: string) {
